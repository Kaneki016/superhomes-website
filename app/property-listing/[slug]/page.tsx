    ...
    useEffect(() => {
        async function loadData() {
            try {
                ...
                // Get property by slug (optimized search)
                let propertyData = await getPropertyBySlug(slug)
                ...
                if (!propertyData) {
                    console.log('Property not found, setting 404 state')
                    setNotFoundState(true)
                    return
                }

                agentData = await getAgentByAgentId(propertyData.agent_id || '')
                similar = await getSimilarProperties(propertyData.id, propertyData.property_type || '', propertyData.state, propertyData.listing_type, propertyData.district)
