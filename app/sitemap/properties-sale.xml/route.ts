 ...
 * Dynamically generates sitemap for all active properties listed for sale
 */
export async function GET() {
    const baseUrl = 'https://superhomes.my'

    try {
        // Fetch properties for sale (limit to 50,000 as per Google's recommendation)
        const properties = await getPropertiesForSitemap('sale', 50000)
...
${propertyUrls.map(entry => `  <url>
    ...
    <priority>${entry.priority}</priority>
        ...
        console.error('Error generating properties-sale sitemap:', error)
